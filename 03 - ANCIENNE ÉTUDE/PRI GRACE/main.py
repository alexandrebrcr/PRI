...
def format_distance_in_meters(distance): 
    ...
    if distance is None:
        return "inconnue"
    # Mettre la distance de base en centimètres sous forme mètres . centimètres
    meters = distance / 100
    return f"{meters:.1f}"
        ...
        while True:
                ...
                if distance is not None:
                    ...
                    if detections:
                        for detection in detections:
                            class_name = camera.get_class_name(detection.ClassID)
                            confidence = detection.Confidence
                            sound.speak(f" {class_name} à distance inconnue")
                            print(f" {class_name} à distance inconnue")
                    else:
                        print("Pas d'objet détecté ni de distance mesurable")
            ...
            # Mode marche 
                ...
                if distance is not None:
                    ...
                    # Si pas de distance valide
                    sound.speak("Distance inconnue")
                    print("Erreur de lecture de distance")

            # Ajout d'une petite pause pour réduire l'utilisation CPU
            time.sleep(0.01)
